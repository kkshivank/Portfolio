...
import { ArrowLeft, PenTool, Hash, Calendar, BookText } from "lucide-react";
...
export default function BlogsPage() {
  ...
  useEffect(() => {
    ...
    const fetchBlogs = async () => {
      try {
        ...
        // Your controller already sorts by createdAt: -1 (newest first)
  ...
  // Get the year of the most recent post (which is the first one due to backend sorting)
  const latestYear = blogs.length > 0 && blogs[0].createdAt 
    ? new Date(blogs[0].createdAt).getFullYear() 
    : new Date().getFullYear();

  // Helper to format dates nicely if they exist in your DB
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <span className="text-sm font-mono text-teal-400 flex items-center">
              <PenTool className="h-3.5 w-3.5 mr-1.5" /> Technical Blog
            </span>
          </div>
        </div>
      </nav>
            ...
            {isLoading ? (
              ...
              [1, 2, 3].map((skeleton) => (
                <div key={skeleton} className="animate-pulse">
                  ...
                  <div className="h-4 bg-slate-800 w-1/4 rounded mb-8"></div>
              ...
              blogs.map((blog) => (
                ...
                >
                  ...
                  {blog.summary && (
                    <div className="mb-8">
                      <p className="text-teal-400/90 text-sm md:text-base font-medium leading-relaxed border-l-2 border-teal-400/50 pl-4">
                        {blog.summary}
                      </p>
                    </div>
                  )}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-6 border-t border-slate-800">
                      <div className="flex flex-wrap gap-2">
                        {blog.tags.map((tag, idx) => (
                          <span 
                            key={idx} 
                            className="flex items-center px-2.5 py-1 bg-slate-950 border border-slate-700/60 rounded text-slate-400 text-xs font-mono tracking-wide uppercase"
                          >
